...
class Terminal extends events.EventEmitter {
  constructor(options = {}) {
    ...
    this.database = new Database(options.terminal, options.path); // throws a lot of stuff
    ...
    // Set up a event hook on tty resize
    this.stdout.on("resize", function() {
      this.columns = this.stdout.columns;
      this.rows = this.stdout.rows;
      
      this.emit("resize", this.columns, this.rows);
    }.bind(this));

    // Event listeners need a reference to a function to remove it and bind creates a new one
  ...
  _parseKey(data) {
    ...
    let matches = this.keysFromBuffer(data, true);
    matches.forEach(function(key) {

      // Parse additional private keys into their own events
      if (key.key.startsWith("_")) {
        let params = [];
        try {
          params = Database.deformat(this.keys[key.key], key.raw);
        } catch (error) {
          return;
        }

        switch (key.key) {
          case "_BEGIN_PASTE": break;
          case "_END_PASTE": break;
          case "_REPORT_CURSOR":
            this.emit("cursor", params[0], params[1]);
            break;
          case "_MOUSE":
            // button is a bitmask of
            // 64: scroll wheel
            // 16: ctrl
            // 8: meta
            // 4: shift 
            // 0-2 is a integer of the button id 0 = primary/wheelup, 1 = middle/wheeldown, 2 = secondary, 3 = mouse moved (no button pressed)
            let [mask, x, y, pressed] = params;
            let button = mask & 0b11;
            let modifiers = {
              ctrl: (mask & 16) > 0,
              meta: (mask & 8) > 0,
              shift: (mask & 4) > 0
  ...
  close() {
    ...
    this.stdin.removeListener("data", this._parseKey);
  ...
  configureColor(id, color) {   
    ...
    return Database.format(this.database.setColor, id, ...color);
  }

  deleteLine(lines) {
    if (this.database.deleteLine === "") {
      return;
    }

    this.write(Database.format(this.database.deleteLine, lines));
  }

  insertLine(lines) {
    if (this.database.insertLine === "") {
      return;
    }

    this.write(Database.format(this.database.insertLine, lines));
  }

  keysFromBuffer(data, deformat = false) {
    if (typeof data !== "string") {
      data = data.toString();
    }
    ...
    while (data.length > 0) {
      ...
      let matches = Object.keys(this.keys).reduce(function(array, key) {
        ...
        for (let i = 0; i < raw.length; i++) {
          ...
          // Variable keys, parse using deformat 
          if (deformat && key.startsWith("_")) {
            // Loop through every substring(0, j) testing each
            // Not efficent at all, but again, I don't care
            for (let j = data.length; j > length; j--) {
              let output = data.substring(0, j);
                if (Database.test(raw[i], output)) {
                array.push({key: key, raw: output});
                length = Math.max(length, j);
              }
            }
          }

          // Static key, check normally
  ...
  moveBy(x = 0, y = 0) {
    ...
    if (y > 0) {
      output += Database.format(this.database.cursorDown, Math.abs(y));
    } else if (y < 0) {
      output += Database.format(this.database.cursorUp, Math.abs(y));
    }
    
    if (x > 0) {
      output += Database.format(this.database.cursorRight, Math.abs(x));
    } else if (x < 0) {
      output += Database.format(this.database.cursorLeft, Math.abs(x));
    }
  ...
  moveTo(x, y) {
    if (x == null) {
      this.write(Database.format(this.database.moveRow, y));
    } else if (y == null) {
      this.write(Database.format(this.database.moveColumn, x));
    } else {
      this.write(Database.format(this.database.moveCursor, y, x));
    }
  }
  ...
  \*/
  open() {
    // Some keys will output differently when this is on
    // The only noted keys that change are directional keys and the number pad +-*/ and enter keys change to an escape code
    // We already have direction keys inputs added, and I don't really care for the number pad so this stays off
    // this.write(this.database.enterKeypad);
    ...
    // Ready for input, start recieving keystrokes
    // Note that we do not need to use bind as it was already done in the constructor
    this.stdin.on("data", this._parseKey);
  }
  ...
  resetBackground() {
    this.write(this.database.resetBackground);
  }
  ...
  resetCursorColor() {
    this.write(this.database.resetCursorColor);
  }

  resetForeground() {
    this.write(this.database.resetForeground);
  }
  ...
  saveCursor() {
    this.write(this.database.saveCursor);
  }

  scrollBy(x) {
    if (this.database.scrollUp === "" || this.database.scrollDown === "") {
      return;
    }

    if (x > 0) {
      // New lines inserted at the bottom
      this.write(Database.format(this.database.scrollUp, Math.abs(x)));
    } else if (x < 0) {
      // New lines inserted at the top
      this.write(Database.format(this.database.scrollDown, Math.abs(x)));
    }
  }
  ...
  setBackgroundTrue() {
    if (this.database.setBackgroundTrue === "" && !this.database.truecolor) {
      return;
    }

    this.write(Database.format(this.database.setBackgroundTrue, ...color));
  }
  ...
  setCursorColor(red, green, blue) {
    if (this.database.setCursorColor === "") {
      return;
    }

    this.write(Database.format(this.database.setCursorColor, red, green, blue));
  }
  ...
  setCursorStyle(style, steady) {
    ...
    this.write(Database.format(this.database.setCursorStyle, style, steady));
  }
  ...
  setForeground(id = -1) {
    ...
    if (id < 0) {
      ...
      this.write(Database.format(this.database.setForeground, id));
    }
  }

  setForegroundTrue(color = -1) {
    if (this.database.setForegroundTrue === "" && !this.database.truecolor) {
      return;
    }

    this.write(Database.format(this.database.setForegroundTrue, ...color));
  }
  ...
  setMouseMode(state) {
    ...
    if (state < 0) {
      ...
      this.write(Database.format(this.database.enterMouse, state));
    }
  }

  setScrollRegion(upper, lower) {
    if (this.database.setScrollRegion === "") {
      return;
    }

    // Pass both paramaters 0/undefined/null to reset, hard-coded in escape string
    this.write(Database.format(this.database.setScrollRegion, upper, lower));
  }
