...
class Terminal extends events.EventEmitter {
  ...
  moveRelative(x = 1, y = 1) {
    if (this.terminfo.capabilities.PARAM_DOWN_CURSOR === "" || this.terminfo.capabilities.PARAM_UP_CURSOR === "" || this.terminfo.capabilities.PARAM_RIGHT_CURSOR === "" || this.terminfo.capabilities.PARAM_LEFT_CURSOR === "") {
      throw new Error("Terminal not capable");
    }
    
    let output = "";
    if (y > 0) {
      output += this.terminfo.capabilities.PARAM_DOWN_CURSOR(Math.abs(y));
    } else if (y < 0) {
      output += this.terminfo.capabilities.PARAM_UP_CURSOR(Math.abs(y));
    }
    
    if (x > 0) {
      output += this.terminfo.capabilities.PARAM_RIGHT_CURSOR(Math.abs(x));
    } else if (x < 0) {
      output += this.terminfo.capabilities.PARAM_LEFT_CURSOR(Math.abs(x));
    }
