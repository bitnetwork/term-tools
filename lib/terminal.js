...
class Terminal extends events.EventEmitter {
  constructor(options = {}) {
    ...
    this.alternateCharsetFallback = {
      "\u2518": "/", // bottom right (can also be +)
      "\u2510": "\\", // upper right (can also be +)
      "\u250c": "/", // upper left (can also be +)
      "\u2514": "\\", // bottom left (can also be +)
      "\u253c": "+", // 4 way
      "\u2500": "-", // horizontal line
      "\u251c": "+", // right facing t
      "\u2524": "+", // left facing t
    ...
    for (let i = 0; i < 26; i++) {
      ...
      this.keys["ctrl" + letter.toUpperCase()] = String.fromCharCode(i + 1);
      this.keys["meta" + letter.toUpperCase()] = "\x1b" + letter;
      this.keys["metaShift" + letter.toUpperCase()] = "\x1b" + letter.toUpperCase();
      this.keys["metaCtrl" + letter.toUpperCase()] = "\x1b" + String.fromCharCode(i + 1);
    }
  ...
  _parseKey(data) {
    ...
    let keys = Object.keys(this.database).filter(function(key) {
      return key.startsWith("key") || /^((begin)|(end)Paste)|(reportCursor)$/.test(key);
    })
    .sort(function(first, second) {
      // It doesn't need to be efficient so I don't care
      return this.database[second].length - this.database[first].length; // Sort by length in descending order (i.e longest first)
    }.bind(this));
