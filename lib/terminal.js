...
class Terminal extends events.EventEmitter {
  constructor(options = {}) {
    ...
    this.stdout.on("resize", function() {
      this.emit("resize", this.stdout.columns, this.stdout.rows);
    }.bind(this));
  ...
  _parseKey(data) {
    ...
    matches.forEach(function(key) {
      ...
      if (key.key.startsWith("_")) {
        let params = [];
        try {
          params = this.database.deformat(this.keys[key.key], key.raw);
        } catch {
          return;
        }
        ...
        switch (key.key) {
          ...
          case "_MOUSE":
            ...
            // 8: meta
            // 4: shift
            // 0-2 is a integer of the button id (0 = wheelup, 1 = wheeldown)
            let [button, x, y, pressed] = params;
            let modifiers = {
              ctrl: button & 16 === 16,
              meta: button & 8 === 8,
              shift: button & 4 === 4
            ...
            // Button is released (it's not possible for scroll wheel to be set here)
            if (!pressed) {
            ...
            // Scrolling
            } else if (button & 64 === 64) {
              // Emit a value of 1 with the sign of the direction we're scrolling
              this.emit("scroll", Object.assign({
                x: x,
                y: y,
                button: button & 0 === 0 ? 0 : 1
              }, modifiers));
            // Mouse moved (either dragging or elsewhat)
            } else {
              this.emit("mouse", Object.assign({
                x: x,
                y: y,
                button: button & 0 === 0 ? 1 : 0 // If non zero, dragging held mouse
              }, modifiers));
            }
  ...
  configureColor(id, color) {   
    ...
    return this.database.format(this.database.setColor, id, ...color);
  }
          ...
          if (deformat && key.startsWith("_")) {
            ...
            for (let j = data.length; j > length; j--) {
              let output = data.substring(0, j);
                if (this.database.test(raw[i], output)) {
  ...
  moveTo(x, y) {
    if (x == null) {
      this.write(this.database.format(this.database.moveRow, y));
    } else if (y == null) {
  ...
  scrollBy(x) {
    ...
    if (x < 0) {
      ...
      return this.database.format(this.database.scrollUp, x);
    } else if (x > 0) {
      return this.database.format(this.database.scrollDown, x);
    }
  }
  ...
  setCursorColor(red, green, blue) {
    ...
    this.write(this.database.format(this.database.setCursorColor, red, green, blue));
  }
  ...
  setCursorStyle(style, steady) {
    ...
    this.write(this.database.format(this.database.setCursorStyle, style, steady));
  }
  ...
  setScrollRegion(upper, lower) {
    ...
    // Both paramaters 0/undefined/null to reset, hard-coded in escape string
    this.write(this.database.format(this.database.setScrollRegion, upper, lower));
  }
