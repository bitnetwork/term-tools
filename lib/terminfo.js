...
class Terminfo {
  constructor(name = process.env.TERM, path = "/usr/share/terminfo/") {
    ...
    let tableOffset = offset + (sizes.string * 2);
    for (let i = 0; i < Math.min(sizes.string, definitions.string.length); i++) {
      let stringOffset = buffer.readInt16LE(offset + (i * 2));
      
      if (stringOffset !== -1) {
        let stringLength = 0;
        while (buffer[tableOffset + stringLength] !== 0) {
          stringLength++;
        }
        
        this.capabilities[definitions.string[i]] = buffer.toString("ascii", tableOffset + stringOffset, tableOffset + stringOffset + stringLength - 1);
      }
    } 
  }
}
