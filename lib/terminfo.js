...
class Terminfo {
  ...
  format(string, ...values) {
    ...
    const regex = /%:?([-+ #])?(\d+|\*)?((?:\.\d+)|(?:\.\*))?([%doxXcscl+\-*/m&|^AO!~i?t;]|p\d+|(?:(?:P|g)(?:[A-Z]|[a-z])))/;
    ...
    while (string.substring(offset).search(regex) !== -1) {
      ...
      switch(match[4].substring(0, 1)) {
        ...
        // %px - push paramater x
        case "p":
          stack.push(values[parseInt(match[4].substring(1), 10) - 1]);
          break;
      }

      // string = string.substring(0, offset) + string.substring(offset).replace(regex, result);
      offset += search + result.length;
      string = string.substring(0, offset - result.length) + result + string.substring(offset - result.length + match[0].length);
    }
    return string;
  }
