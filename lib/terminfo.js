...
class Terminfo {
  ...
  format(string, ...params) {
    ...
    const regex = /%:?([-+ #0])?(\d+|\*)?((?:\.\d+)|(?:\.\*))?([%doxXcsl+\-*/m&|^AO!~i?t;]|(?:p[1-9])|(?:'[^]')|(?:{\d+})|(?:(?:P|g)(?:[A-Z]|[a-z])))/;
    ...
    while (string.substring(offset).search(regex) !== -1) {
      ...
      switch(match[4].substring(0, 1)) {
        ...
        case "/":
          ...
          stack.push(final === Infinity ? 0 : final); // Divide by zero and other stuffes
          break;
        // %m - push modulus of pop over pop
        case "m":
          result = (stack.pop || 0) / (stack.pop || 0); // Fuck scopes in switch statements
          stack.push(final === Infinity ? 0 : final);
          break;
