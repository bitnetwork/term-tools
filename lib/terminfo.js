...
class Terminfo {
  constructor(name = process.env.TERM, dir = "/usr/share/terminfo/", options = {}) {
    let buffer;
    ...
    for (let i = 0; i < Math.min(sizes.string, definitions.string.length); i++) {
      ...
      if (stringOffset !== -1) {
        ...
        this.capabilities[definitions.string[i]] = string.search(/%/) !== -1 && options.functionParams !== false ? function(...params) {_formatString(string, ...params);} : string;
      } else {
      ...
      if (string[i] === "%") {
        switch (string[i + 1]) {
          case "%":
            ...
            break;
          case "d":
            output += Math.floor(params[param]);
            param++;
            break;
          case "i":
            output += params[param];
            ...
            break;
          case "o":
            output += string[param].toString(8);
            param++;
            break;
          case "p":
            param = string[i + 2];
            ...
            break;
          case "s":
            output += string[param].substring(0, string[param].length - 1);
            param++;
            break;
          case "S":
            output += string[param];
            ...
            break;
          case "u":
            output += string[param]; // we shouldn't have to deal with large numbers with scientific notation yet
            param++
            break;
          case "x":
            output += string[param].toString(16).toLowerCase();
            param++;
            break;
          case "X":
            output += string[param].toString(16).toUpperCase();
            param++;
            break;
        }
        i++;
