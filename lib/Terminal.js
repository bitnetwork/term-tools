...
class Terminal extends events.EventEmitter {
  ...
  setStyle(styles = {}) {
    ...
    if ((this.terminfo.ENTER_ITALICS_MODE === "" || this.terminfo.EXIT_ITALICS_MODE === "") && styles.italics) {
      throw new NamedError("NotCapable", "Terminal not capable");
    }

    if (this.terminfo.EXIT_ATTRIBUTES === "" && !use && !use1 && !styles.italics) {
      throw new NamedError("NotCapable", "Terminal not capable");
    }
    ...
    output += styles.italics ? this.terminfo.ENTER_ITALICS_MODE : this.terminfo.EXIT_ITALICS_MODE;

    output += !use && !use1 && !styles.italics ? this.terminfo.EXIT_ATTRIBUTE_MODE : "";
