...
class Terminfo {
  constructor(name = process.env.TERM, dir = path.join(__dirname, "vendor", "terminfo"), options = {}) {
    ...
    // Check out terminfo(5) for capability lists
    let offset = 0;
    let definitions = {
      boolean: [
        "AUTO_LEFT_MARGIN",
    ...
    /* 
     * Terminfo format
     * Check out term(5) for file format
     *
     * Segements
     * Magic number (header)
     * Sizes (header)
     ...
     * String lookup table
     * String table
     * Extended sizes (extended header)
     * Boolean extended table (extended)
     * Number extended table (extended)
     * String lookup extended table (extended)
     * String extended table (extended)
     * Name lookup table (extended)
     */
    ...
    // Make unknown values be defined
    // bool: null, number: -1, string: ""
    
    for (let i = 0; i < /*Math.min(sizes.boolean, */definitions.boolean.length; i++) {
      let value = buffer.readInt8(offset + i);
      this[definitions.boolean[i]] = i < sizes.boolean ? Boolean(value) : null;
    }
    offset += sizes.boolean;
    offset += offset % 2; // A null byte will need to be skipped if the current offset is odd

    for (let i = 0; i < /*Math.min(sizes.number, */definitions.number.length; i++) {
      let number = buffer.readInt16LE(offset + (i * 2));
      this[definitions.number[i]] = i < sizes.number ? number : -1;
    }
    offset += sizes.number * 2;

    for (let i = 0; i < /*Math.min(sizes.string, */definitions.string.length; i++) {
      if (i >= sizes.string) {
        this[definitions.string[i]] = "";
        continue;
      }
      ...
      if (stringOffset !== -1) {
        ...
        this[definitions.string[i]] = string;
      } else {
        this[definitions.string[i]] = "";
      }
    }
    offset += sizes.string * 2;
    offset += sizes.table;

    this.extended = false;

    // Check out term(5) for file format
    // Check if we've reached the end of the file, and if not assume extended syntax
    if (offset < buffer.byteLength) {
      // We are now using terminfo extended syntax
      this.extended = true;
      
      sizes.extendedBoolean = buffer.readInt16LE(offset += 2);
      sizes.extendedNumber = buffer.readInt16LE(offset += 2);
      sizes.extendedString = buffer.readInt16LE(offset += 2);
      sizes.extendedTable = buffer.readInt16LE(offset += 2);
      sizes.extendedTableOffset = buffer.readInt16LE(offset += 2);

      let extendedBooleans = [];
      for (let i = 0; i < sizes.extendedBoolean; i++) {
        let value = buffer.readInt8(offset + i);
        extendedBooleans.push(i < sizes.extendedBoolean ? Boolean(value) : null);
      }
      offset += sizes.extendedBoolean;
      offset += offset % 2; // A null byte will need to be skipped if the current offset is odd


      let extendedNumbers = [];
      for (let i = 0; i < sizes.extendedNumber; i++) {
        let number = buffer.readInt16LE(offset + (i * 2));
        extendedNumbers.push(i < sizes.number ? number : -1);
      }
      offset += sizes.extendedNumbers * 2;
      offset += offset % 2; // A null byte will need to be skipped if the current offset is odd
  ...
  format(string, ...params) {
    ...
    while (string.substring(offset).search(regex) !== -1) {
      // [matched string, flags -+ #, width 0-9*, .precision, specifier for doxXs only, specifier + parameters]
      let match = string.substring(offset).match(regex);
      let search = string.substring(offset).search(regex);
      let output = "";
      
      let flags = match[1];
      let width = match[2]; // pad result with spaces if it is smaller than this length
      let precision = match[3]; // pad result with spaces if it is smaller than this length; this takes priority over width
      let specifier = match[4] !== undefined ? match[4] : match[5];

      let value1, value2;
      
      // specifiers
      switch(specifier.substring(0, 1)) {
        // %% - escape precent sign
        case "%":
          output = "%";
          break;
        ...
        case "p":
          stack.push(params[parseInt(specifier.substring(1, 2), 10) - 1]);
          break;
        // %P[a-z] - set dynamic variable x to pop
        // %P[A-Z] - set static variable x to pop
        case "P":
          this.variables[specifier.substring(1, 2)] = stack.pop();
          break;
        // %g[a-z] - push dynamic variable x
        // %g[A-Z] - push static variable x
        case "g":
          stack.push(this.variables[specifier.substring(1, 2)]);
          break;
        // %'c' - char constant c (do we push it on the stack?)
        case "\'":
          stack.push(specifier.substring(1, 2));
          break;
        // %{nn} - integer constant nn (do we push this also?)
        case "{":
          stack.push(parseInt(specifier.substring(1, specifier.length - 1), 10));
          break;
      ...
      if (flags !== undefined) {
        for (let i = 0; i < flags.length; i++) {
          ...
          switch (flag) {
            ...
            case "+":
              if (output.search(/-/) === -1) { // number is not negative
                output = "+" + output;
              }
              break;
            // % [doxX] - if no sign is to be written, a space is written instead
            case " ":
              if (output.search(/-|\+/) === -1) { // check if a sign does not exist
                output = " " + output;
              }
              break;
            ...
            case "#":
              ...
              if (specifier.substring(0, 1) === "o") {
                output = "0" + output;
              } else if (specifier.substring(0, 1) === "x") {
                output = "0x" + output;
              } else if (specifier.substring(0, 1) === "X") {
                output = "0X" + output;
              }
      ...
      // precision - expand numeric value to n places with zeros
      // %.n[doxX]
      if (precision !== undefined) {
         let amount = parseInt(precision.substring(1), 10) - output.match(/\d|[a-f]|[A-F]/g).length;
         if (amount > 0) {
           let zero = output.search(/\d/);
           output = output.substring(0, zero - 1) + "0".repeat(amount) + output.substring(zero)
         }
      }
      
      // width
      // %n[doxXs] - expand result to match at least n characters with spaces
      if (width !== undefined) {
        let amount = parseInt(width, 10) - output.length;
        if (amount > 0) {
          output = leftJustify ? output + " ".repeat(amount) : " ".repeat(amount) + output;
        }
      }
