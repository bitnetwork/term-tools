...
\|/
/|\ new Database([terminal], [termpath])
\|/
/|\ <String|Buffer> terminal          Either a file name or file buffer to parse.
\|/ <String|Array[String]> termpath   A (list of) search director[y|ies] used as a path.
/|\
\|/ throws NotFoundError  On terminal not found
...
class Database {
  constructor(terminal = process.env.TERM, termpath = [
    ...
    if (typeof terminal === "string") {
      ...
      for (let i = 0; i < termpath.length; i++) {
        ...
        try {
          // throws Error
          json = fs.readFileSync(path.join(termpath[i], terminal) + ".json");
          found = true;
          break;
        } catch (error) {
      ...
      if (!found) {
        throw new NotFoundError(`Terminal ${terminal} not found in path ${termpath}.`);
      }
    
    } else if (terminal instanceof Buffer) {
      json = terminal.toString();
    }
  ...
  /|\
  \*/
  static deformat(program, output) {    
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      switch (operator[0]) {
        ...
        case "P":
          variables[operator[1]] = stack.pop();
          break;
          
        // %g[a-z] - push dynamic variable x
        // %g[A-Z] - push static variable x
        case "g":
          stack.push(variables[operator[1]]);
          break;
